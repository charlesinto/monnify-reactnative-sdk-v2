...
class AccountTransferViewController: BaseViewController {
    ...
    @IBOutlet weak var moneyTransferredButton: GradientButton!
    ...
    @IBAction func checkTransferStatusButton(_ sender: UIButton) {
        if(moneyTransferredButton.isLoading){
            return
        }
        moneyTransferredButton.showLoader(userInteraction: true)
        let alert = UIAlertController(title: "Alert", message: "Transaction is pending confirmation", preferredStyle: .alert)

        DispatchQueue.main.asyncAfter(deadline: .now() + 4)
        {
            self.moneyTransferredButton.hideLoader()
