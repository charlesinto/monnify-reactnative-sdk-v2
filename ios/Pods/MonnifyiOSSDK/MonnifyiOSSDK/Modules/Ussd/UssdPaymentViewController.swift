//
//  UssdPaymentViewController.swift
//  MonnifyiOSSDK
//
...
class UssdPaymentViewController: BaseViewController {
    
    var viewModel: UssdPaymentViewModel!
    ...
    fileprivate var presenter: UssdPaymentPresenter!
    ...
    private func setupView() {
        ...
        presenter = UssdPaymentPresenter(view: self,
...
extension UssdPaymentViewController : UssdPaymentViewDelegate {
    ...
    func showUssdTransferDetails(viewModel: UssdPaymentDetailsViewModel) {
        ...
        let vc = viewController(type: UssdPaymentDetailsViewController.self)
...
extension UssdPaymentViewController : UIPickerViewDataSource, UIPickerViewDelegate {
...
extension UssdPaymentViewController: ToolbarPickerViewDelegate {
    ...
    fileprivate func refreshViewForSelection() {
        guard let bank = selectedBank, let ussdCode = bank.baseUssdCode?.dropFirst().dropLast() else {
            return
        }
        
        self.selectBankDropdownTextField.text = bank.name?.capitalized
                
        self.presenter.initializeUssdPaymentDetails(String(ussdCode), bank.name?.capitalized ?? "")
